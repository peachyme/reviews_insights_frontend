...
const columns = [
  { field: 'name', headerName: 'Business Name', width: 300 },
  { field: 'num_reviews', headerName: 'Number of Reviews', width: 200 }
];

const MostReviewed = () => {
  const { data: fetchedBusinesses, isLoading: isLoadingBusinesses, error: businessesError } = useGetMostReviewedBusinessesQuery();
  ...
  useEffect(() => {
    if (fetchedBusinesses) {
      ...
      const reviews = topBusinesses.map((business) => business.num_reviews);
